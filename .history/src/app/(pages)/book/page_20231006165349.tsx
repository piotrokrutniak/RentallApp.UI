...
export default function BookingPage(){
    ...
    function Submit(){
        if(ValidateUserDetails()){
            PostReservation(booking).then(x => console.log(x))
        }
    }
    ...
    return(
        ...
        <section id="header-section" className='max-w-7xl bg-black/90 mx-auto w-full p-6 lg:p-14 rounded-xl mt-4 shadow-md shadow-black/40'>
            <div className="flex flex-col gap-5 text-white">
                 {vehicles.map((x, key) => <VehicleItem booking={booking} setSelected={setBooking} vehicle={x} startDate={booking.startDate} endDate={booking.endDate} key={key}/>)}
            </div>
        </section>}
        ...
        <section id="header-section" className='max-w-7xl bg-black/90 mx-auto w-full p-6 lg:p-14 rounded-xl mt-4 shadow-md shadow-black/40'>
            <div className="relative lg:w-1/2 left-0 flex flex-col gap-5 pt-3 overflow-x-auto m-auto justify-around select-none md:pb-10 text-white">
                ...
                {validation.apiError == "" ? <></> : <h3 className="text-vermilion-400 text-base flex flex-row-reverse">{validation.apiError}</h3>}
            </div>
        </section>
        </main>
    )
