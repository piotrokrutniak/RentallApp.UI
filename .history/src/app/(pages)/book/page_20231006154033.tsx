...
export default function BookingPage(){
    ...
    function UpdateLocation(text: string, location: number){
        setSearchLocationValue(text)
        setBooking({...booking, locationId: location})
    }
        ...
        <section id="header-section" className='max-w-7xl bg-black/90 mx-auto w-full p-6 lg:p-14 rounded-xl mt-4 shadow-md shadow-black/40'>
            ...
            <div className="relative bottom-0 w-full h-fit left-0 flex flex-col gap-2 pt-3 overflow-x-auto m-auto justify-around select-none md:pb-10 text-white">
                ...
                <FormInput minValue={GetDate()} onChange={UpdateStart} 
                    onBlur={() => setValidation({...validation, startDate: booking.endDate == ""})} validationMessage="This field is required." 
                    className="w-full" type="date" label="Start Date"/>
                <FormInput minValue={booking.startDate ?? GetDate()} onChange={UpdateEnd} validationMessage="This field is required."
                    onBlur={() => setValidation({...validation, startDate: booking.startDate == ""})} 
