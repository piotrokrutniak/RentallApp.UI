    ...
    return(
        ...
        <section id="header-section" className='max-w-7xl bg-black/90 mx-auto w-full p-6 lg:p-14 rounded-xl mt-4 shadow-md shadow-black/40'>
            <div className="flex flex-col gap-5 text-white">
                {vehicles.length > 0 && vehicles.map((x, key) => <VehicleItem setSelected={setBooking} vehicle={x} startDate={booking.startDate} endDate={booking.endDate} key={key}/>)}
            </div>
        </section>
...
function VehicleItem({vehicle, startDate, endDate, setSelected}:{
    vehicle: Vehicle,
    startDate: string,
    endDate: string;
        ...
        <div className="w-full p-4 rounded-md bg-slate-500/40 flex gap-5">
            ...
            <div className="text-white w-full flex flex-col justify-between gap-1">
                ...
                <div className="flex justify-between place-items-center">
                    ...
                    <Button disabled={!available} onClick={() => setSelected(x => [...x, vehicleId: "vehicle.id"])} className={`bg-green-500/80 hover:bg-green-500/90 text-white w-full md:w-fit mr-0 ml-auto text-xl  active:opacity-80 shadow-black/40 hover:shadow-md
                    justify-center`}
                    onClick={undefined}>
                    Select
                    </Button>
