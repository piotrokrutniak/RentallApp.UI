    ...
    return(
        <div className={`${className ?? ""} flex flex-col relative`}>
            ...
            <input type={type || "text"} defaultValue={defaultValue || ""} placeholder={placeholder ?? ""}
                ...
                {validationResult ? <></> : 
                <div className="top-0 relative bg-red-200">
                    <h3 className={`${messageClassName ?? ""} text-vermilion-400 text-base flex flex-row-reverse absolute -bottom-6 right-0`}>{validationMessage}</h3>
                </div>}
