    ...
    return(
        <div className={`${display ? "" : "hidden"} absolute bg-slate-600 z-20 rounded-md w-1/2 right-0 mt-2 h-44 shadow-md shadow-black/40 overflow-clip`}>
            <div className="overflow-y-scroll h-44">
                {loading && <p>Loading... <BsArrowClockwise className="animate-spin"/></p>}
                {results.length > 0 ? results.map(x => <Result text={x} onClick={handleClick}/>) : <p className="p-3">No results found.</p>}
            </div>
        </div>
    )
}
